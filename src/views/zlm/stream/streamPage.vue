<template>
  <div class="custom-div">
    <a-form layout="inline" class="custom-form">
      <a-row :gutter="16">
        ...
        <a-col :xs="24" :sm="12" :md="6">
          <a-form-item>
            <a-button type="primary" @click="fetchMediaList">查询</a-button>
            <a-button style="margin-left: 5px;" type="info" @click="resetSearch">重置</a-button>
          </a-form-item>
        </a-col>
      </a-row>
    </a-form>
    ...
    <a-table :columns="columns" :data-source="mediaList" :loading="loading" ref="table">
      ...
      <div slot="action" slot-scope="text, record">
        <span v-if="!record.isRecordingMP4">
          <a @click="openMp4BackAction(text, record)"><a-icon type="poweroff"/>开启MP4录像</a>
          <a-divider type="vertical"/>
        </span>
        <span v-else>
          <a-popconfirm title="确定关闭MP4录像吗？" @confirm="stopRecordMp4Action(text, record)">
            <a style="color:red"><a-icon type="poweroff"/> 关闭MP4录像</a>
          </a-popconfirm>
          <a-divider type="vertical"/>
...
export default {
  ...
  methods: {
    ...
    stopRecordMp4Action(text, record) {
      stopRecordMp4({app: record.app, stream: record.stream}).then(res => {
        if (parseInt(res.code) === 0 && res.result) {
          this.$message.success("流停止录制MP4成功");
          this.fetchMediaListTimeout();
        } else {
          this.$message.warning("流停止录制MP4失败，请稍后重试");
        }
      }).catch(err => {
        this.$message.error(err.message || "流停止录制MP4失败");
      });
    },
    ...
    generateUrls(item) {
      ...
      const baseUrls = {
        ...
        rtmp: [
          ...
          `http://192.168.1.119:8080/${app}/${streamId}.live.flv`,
          `https://192.168.1.119:8080/${app}/${streamId}.live.flv`,
          ...
          `ws://127.0.0.1${port}/${app}/${streamId}.live.flv?vhost=${host}`,
          `wss://127.0.0.1${port}/${app}/${streamId}.live.flv?vhost=${host}`
        ],
        hls: [
        ...
        ts: [
          ...
          `ws://127.0.0.1${port}/${app}/${streamId}.live.ts?vhost=${host}`,
          `wss://127.0.0.1${port}/${app}/${streamId}.live.ts?vhost=${host}`
        ],
        fmp4: [
          ...
          `ws://127.0.0.1${port}/${app}/${streamId}.live.mp4?vhost=${host}`,
          `wss://127.0.0.1${port}/${app}/${streamId}.live.mp4?vhost=${host}`
        ]
      };
      // 获取该流源支持的URL类型
      const supportedUrls = [];
